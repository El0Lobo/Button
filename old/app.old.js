    ...
    function logExportSnapshot(label, root) {
      const scope = root || document;
      let canvasEl = null;
      if (scope && scope.nodeType === 1 && scope.id === "a4canvas") {
        canvasEl = scope;
      } else if (scope && scope.querySelector) {
        canvasEl = scope.querySelector("#a4canvas");
      } else {
        canvasEl = a4canvas;
      }
      if (!canvasEl) {
        console.warn(`[export] ${label}: missing #a4canvas in snapshot scope`);
        return;
      }
      const canvasRect = canvasEl.getBoundingClientRect();
      const wrapperRect = canvasWrapper ? canvasWrapper.getBoundingClientRect() : null;
      const firstSlot = canvasEl.querySelector(".button-slot");
      const firstContent = canvasEl.querySelector(".button-content");
      const firstImg = canvasEl.querySelector(".button-content img.button-image");
      const firstCutline = canvasEl.querySelector(
        ".overlay-cutline-custom[style*=\"display: block\"], .overlay-cutline-default[style*=\"display: block\"]"
      );
      const slotRect = firstSlot ? firstSlot.getBoundingClientRect() : null;
      ...
      const cutlineRect = firstCutline ? firstCutline.getBoundingClientRect() : null;
      ...
      if (cutlineRect && slotRect) {
        console.log("cutlineRect", cutlineRect);
        console.log("cutlineOffset", {
          offsetX: cutlineRect.left - slotRect.left,
          offsetY: cutlineRect.top - slotRect.top
        });
      }
      console.log("customCutline", {
        enabled: !!customCutlineToggle?.checked,
        diameter: customCutlineDiameter?.value
      });
      console.log("canvasZoom", canvasZoomSlider?.value);
      console.log("buttonScale", scaleSlider?.value);
      console.groupEnd();
    }
    ...
    function applyExportStyles(root, logoSource) {
      ...
      root.querySelectorAll(".button-content").forEach((content) => {
      ...
      root.querySelectorAll(".button-bg-color").forEach((bg) => {
      ...
      root.querySelectorAll(".overlay-cutbox, .overlay-cutline-default, .overlay-cutline-custom, .overlay-outer, .overlay-inner, .overlay-middle").forEach((el) => {
    ...
    function syncCloneFromLive(cloneRoot) {
      // No-op: clone layout now uses standard centered transforms.
    }
    ...
    function exportSheet(kind) {
      ...
      waitForImages(imgs).then(() => {
        logExportSnapshot("before export");

        const originalId = a4canvas.id;
        a4canvas.id = "a4canvas-live";

        const clone = a4canvas.cloneNode(true);
        clone.id = originalId;
        ...
        const exportWidth = clone.scrollWidth || clone.clientWidth || 0;
        const exportHeight = clone.scrollHeight || clone.clientHeight || 0;
        exportHost.style.width = exportWidth ? `${exportWidth}px` : "auto";
        exportHost.style.height = exportHeight ? `${exportHeight}px` : "auto";
        exportHost.appendChild(clone);
        document.body.appendChild(exportHost);

        applyExportStyles(clone, acmeLogo);
        waitForImages(clone.querySelectorAll("img"))
          .then(() => {
            recomputeCloneImageScales(clone);
            syncCloneFromLive(clone);
            logExportSnapshot("clone before render", clone);
            return html2canvas(clone, {
              ...
              x: 0,
              y: 0,
              width: exportWidth || undefined,
              height: exportHeight || undefined,
              windowWidth: exportWidth || undefined,
              windowHeight: exportHeight || undefined,
            });
          })
          .then((canvas) => {
            console.log("[export] result canvas size", { width: canvas.width, height: canvas.height });
            if (kind === "pdf") {
